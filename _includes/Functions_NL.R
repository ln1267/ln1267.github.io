...
fetchANULAI = function(lat, long, buffer = 0.02, start_year = 2000, end_year = 2022) {
    ...
    process_year_data <- function(year, lat, long, buffer) {
        ...
        if (length(dim(lai_data))==2) lai_data <- apply(lai_data, 2, mean, na.rm = TRUE)
		if (length(dim(lai_data))==3) lai_data <- apply(lai_data, 3, mean, na.rm = TRUE)
...
#' @param nameExt A character string specifying the appendix name that corresponding files are expected to be found.
...
f_check_processed_files = function(parent_directory, out_path = NULL,nameExt=".tif") {
  ...
  check_processed_files <- function(file_path, out_path) {
    ...
    cog_file_name <- paste0(file_name, nameExt)
